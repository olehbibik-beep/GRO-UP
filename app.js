...
// 🔥 ПРОГРАММА СОБРАНИЯ (БЕЗ РАМОК, КРУПНЫЕ ШРИФТЫ, ВЫХОДНЫЕ ДОБАВЛЕНЫ)
...
function buildScheduleCards(d, myName, currentWeekStr) {
    ...
    const row = (title, person) => {
        ...
        return `
            <div class="flex items-center justify-between py-2 px-1 border-b border-slate-300/50 last:border-0">
                <span class="text-sm md:text-base font-bold text-slate-700 flex-1 pr-1 leading-tight">${partCounter++}. ${translateDbString(title)}</span>
    ...
    const rowUnnumbered = (title, person) => {
        ...
        const nameColor = isMe ? `text-rose-600 bg-rose-100 px-2 py-0.5 rounded shadow-sm` : 'text-slate-800';
        return `
            <div class="flex items-center justify-between py-2 px-1 border-b border-slate-300/50">
                <span class="text-xs md:text-sm font-black text-slate-600 flex-1 pr-1">${translateDbString(title)}</span>
                <div class="shrink-0 max-w-[60%] flex items-center justify-end text-right">
                    <span class="text-base md:text-lg font-black ${nameColor} truncate leading-tight">${person || '-'}</span>
                </div>
            </div>
    ...
    const minRows = (d.ministryParts || []).map((m) => {
        ...
        const names = `<div class="flex flex-col text-right truncate w-full items-end">
            ...
            ${m.assistant ? `<span class="text-xs md:text-sm font-bold ${assistCol} truncate mt-0.5 w-full">${window.t('assistant_short')} ${m.assistant}</span>` : ''}
        </div>`;

        return `
            <div class="flex items-center justify-between py-2 px-1 border-b border-slate-300/50 last:border-0">
                <span class="text-sm md:text-base font-bold text-slate-700 flex-1 pr-1 leading-snug">${partCounter++}. ${translateDbString(m.type || window.t('part'))}</span>
    ...
    return `
        <div ${isCurrentWeek ? 'id="current-week-card"' : ''} class="w-[88vw] md:w-[calc(50%-0.75rem)] shrink-0 snap-center flex flex-col bg-transparent mb-2">
            <div class="bg-slate-800 p-2 md:p-3 text-center rounded-t-lg shrink-0 border-b border-slate-900 shadow-sm">
                <span class="text-[9px] md:text-[10px] font-black ${statusColor} uppercase tracking-widest block mb-0.5">${weekStatus}</span>
            ...
            <div class="flex-grow flex flex-col pb-2">
                ...
                <div class="bg-[#0d9488] text-white py-1.5 px-2 mt-1 flex items-center shadow-sm">
                ...
                <div class="bg-[#d97706] text-white py-1.5 px-2 mt-1 flex items-center shadow-sm">
                    <span class="text-[10px] md:text-xs font-black uppercase tracking-widest leading-none">${window.t('ministry_skills')}</span>
                </div>
                ${minRows}

                <div class="bg-[#b91c1c] text-white py-1.5 px-2 mt-1 flex items-center shadow-sm">
                    <span class="text-[10px] md:text-xs font-black uppercase tracking-widest leading-none">${window.t('christian_living')}</span>
                </div>
                ${livRows}
                
                <div class="flex items-center justify-between py-2.5 px-1 border-b border-slate-300/50">
                    <div class="flex flex-col flex-1 pr-1">
                        <span class="text-sm md:text-base font-bold text-slate-700 leading-tight">${cbsNum}. ${window.t('congregation_bible_study')}</span>
                    ...
                    <div class="shrink-0 max-w-[60%] flex items-center justify-end text-right">
                        <div class="flex flex-col items-end truncate w-full">
                            ...
                            ${d.mw_cbs_reader ? `<span class="text-xs md:text-sm font-bold ${readCol} truncate mt-0.5 w-full">${window.t('reader')} ${d.mw_cbs_reader}</span>` : ''}
                        </div>
                    </div>
                </div>
        ...
        <div class="w-[88vw] md:w-[calc(50%-0.75rem)] shrink-0 snap-center flex flex-col bg-transparent mb-2">
            ...
            <div class="flex-grow flex flex-col pb-2">
                ...
                <div class="flex items-center justify-between py-2.5 px-1 border-b border-slate-300/50">
                    <div class="flex flex-col flex-1 pr-1">
                        <span class="text-sm md:text-base font-bold text-slate-700 leading-tight">${window.t('watchtower_study')}</span>
                    </div>
                    <div class="shrink-0 max-w-[60%] flex items-center justify-end text-right">
                        <div class="flex flex-col items-end truncate w-full">
                            <span class="text-base md:text-lg font-black ${we_wt_cond} truncate leading-tight w-full">${d.we_wt_conductor || '-'}</span>
                            ${d.we_wt_reader ? `<span class="text-xs md:text-sm font-bold ${we_wt_read} truncate mt-0.5 w-full">${window.t('reader')} ${d.we_wt_reader}</span>` : ''}
                        </div>
                    </div>
                </div>
